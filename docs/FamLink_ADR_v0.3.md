...
**ADR-04 Database Architecture** [LOCKED — REVISIT PHASE 2]
...
**ADR-07 Notifications Infrastructure** [LOCKED]
...
**ADR-08 Hosting & Infrastructure** [LOCKED — REVISIT PHASE 2]
...
**ADR-09 Real-Time & Chat** [LOCKED — REVISIT PHASE 2]
...
| 5 | Invitation System | 3, 4 | Email + SMS invite delivery, guest token generation, RSVP capture | Phase 1 |
...
| 4 | COPPA compliance: parental consent flow design for minor profiles? | OPEN | Invitation system for minor profiles (Build Order 5) | Partially resolved: age_gate_level enum and guardian_person_id added (v0.3). MVP adults-only for accounts and comms; minors are passive graph nodes. Full COPPA consent flow requires legal input before any minor account functionality is built. |
...
| 2 | Resolve Open Q#4 (COPPA compliance flow) — requires legal input before any minor account functionality is built | Legal guidance documented; COPPA consent flow designed and locked in ADR |
| 3 | Resolve Open Q#2 (subscription tier member cap) — needed before Phase 3 payment build | Monetization model member cap locked; Stripe integration plan updated |
