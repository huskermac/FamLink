...
8. **Consent gates a *reachable autonomous party*, not the mechanical membership row (Steve, 2026-08-07 — narrows decision 6 and §3.2/§6.2/§6.3).** Building out your own family with authored records is data entry, not an act against another person. The discriminator is **whether the target Person can be reached / acts autonomously**, decided by two facts on the `Person`:
   ...
   Consequences carried into the plan: (a) the existing unconsented `POST /families/:familyId/members` (families.ts) is **kept only for the no-consent case** and must **reject / route through a `LinkRequest`** any target that has an account or a contact detail (provenance-gated — see decision 9); (b) seat billing decouples from the mutation entirely — see decision 10; (c) the **passive→active** consent moment (a passive record claiming an account) is **already handled at signup by CIF Plan B** (the Clerk `user.created` consolidation/merge) and is **not** re-implemented here.
9. **Direct-add provenance — a `Person` gains an authoring owner (Steve, 2026-08-07, from the round-2 council).** The consent-gate exemption for "passive, no contact = data entry" is only safe if the record was genuinely **authored by the requester's family** — otherwise an admin could attach a *foreign* passive person by guessing their id and gain their PII. `Person` gains a **`createdByFamilyGroupId String?`** column (logical, set whenever a passive `Person` is created in a family context — onboarding member creation, `POST /persons` by an existing user). **Data-entry direct-add is allowed only when the target's `createdByFamilyGroupId` equals the requester's family** (in addition to passive + no-contact). Migration backfills existing passive persons to a family they are currently a member of (prod is tiny — 12 persons). A `Person` with no owner (or a foreign owner) can only be added via a consented `LinkRequest`.
10. **Seat billing is decoupled from consent and reconciled from actual membership (Steve, 2026-08-07, from the round-2 council).** Tying a Stripe seat charge to the add/accept event is race-prone and non-recoverable (request-time authorization goes stale by accept time; a Stripe failure after the grant commits strands an accepted membership). Instead: **the consent/accept flow performs no inline Stripe calls and never blocks acceptance on billing.** Seat count is **reconciled from the family's true active-member count** by an **idempotent reconciliation** (sets the absolute Stripe seat quantity) triggered on every membership change (add / remove / suspend / CIF activation) and reconvergent via the `customer.subscription.updated` webhook. A request-time cost *disclosure* to the requesting admin is allowed (informational, non-binding), but it is not a gate and never a charge. This supersedes decision 8 consequence (b) ("moves to the acceptance step") and the existing per-mutation `checkSeatExpansion` 402-confirm on member-add. **Building/verifying that reconciliation is its own slice (may precede or accompany PR 2); the consent PR must not call Stripe.**
