...
| Branch | `main` (W3a-UI-mobile PR 1 merged; docs checkpoint on top of `d1abca7`). |
| Checkpoint | **W3a-UI-mobile PR 1 (participant slice) MERGED (2026-07-06, PR #9, squash `d1abca7`):** 3 additive API changes (atomic `POST /items/:itemId/claim`, `myRsvp` on the foreign DTO, `GET /events/participating` discovery w/ multi-family dedupe) + mobile participant surfaces (foreign-event discovery merge w/ "Guest" badge, `OwnEventDetail`/`ForeignEventDetail` split, items on the `/items` model fixing the broken potluck claim, isError-first stale-cache guard) + **first mobile CI job**. Whole-branch opus review READY TO MERGE, no Critical/Important. **Also live since 2026-07-02:** production billing (Stripe live), R2 bucket-scoped token, `NODE_ENV=production`. **Next: W3a-UI-mobile PR 2 (organizer slice).** |
| Local verification | PASS (W3a-UI-mobile PR 1): full API suite **423/423**, full web suite **pass** (coverage gate green at **87.29%** lines, threshold 80), mobile suite **37/37**, repo-root `type-check` 6/6 clean, `lint` 0 errors (pre-existing warnings only), `git diff --check` clean. Mobile now runs in CI (lint + type-check + jest). |
...
4. **W3a-UI-mobile** — split into two PRs. **PR 1 (participant slice) DONE, MERGED** (PR #9, squash `d1abca7`; branch deleted). **PR 2 (organizer slice — mobile invite screen + participant management) is next.**
...
**W3a-UI-mobile PR 2 (organizer slice)** - the committed follow-on to the merged PR 1. Mobile **invite screen** (family members + cross-family suggestions with admin-gated role toggle + external guests) and **participant management** (participant list + admin revoke/role) on the own-event detail. The API is fully in place (invite/suggestions/participants/revoke/role endpoints from W3a-API); the web invite page + PR-1 mobile patterns are the reference. Design spec §5 (`docs/superpowers/specs/2026-07-06-w3a-ui-mobile-design.md`) already scopes it; write a PR-2 plan next. Note the client `canAdmin` gating uses family roles only (`useMyFamilies`), matching what web ships.
