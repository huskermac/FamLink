...
- **Backlog from M3 review (not started):** (1) cross-family invitation visibility — an invited+linked person should see the "foreign" event in their upcoming list unless declined (net-new feature; council-reviewed design at `docs/FamLink_Design_Cross_Family_Invitation_Visibility.md`); (2) person-photo URL trust — `updatePersonPhotoUrl` accepts a client URL, same class as the event-photo fix.
